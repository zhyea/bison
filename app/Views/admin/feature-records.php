<div class="container main">
    <div class="page-header">
        <h3>
            ...
            <span class="tag"><a href="<?= $siteUrl ?>/admin/feature/list">返回专题列表</a></span>
        </h3>
    </div>
...
<script>
    ...
    $("#featureRecordSelectInput").bsSuggest({
        clearable: true,
        url: "<?=$siteUrl?>/admin/work/suggest?key=",
        showHeader: false,
        ...
        $.post('<?=$siteUrl?>/admin/feature/record/add/<?=$id?>/' + data.id,
    ...
    $("#btnDelete").on("click", function () {
        ...
        if (rows.length > 0) {// rows 主要是为了判断是否选中，下面的else内容才是主要
            ...
            sendBootstrapTableRequest($table, 'post', '<?=$siteUrl?>/admin/feature/records/delete', ids);
        }
    });
    ...
    function changeOrder(id, step) {
        ...
        sendBootstrapTableRequest($table, 'post', '<?= $siteUrl?>/admin/feature/record/change-order/' + id, step);
    }
</script>
